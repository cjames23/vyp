...
use vyp_api::{MarkerEnvironment, Requirement, VypVersion};
...
pub fn wheel_filter_disabled() -> bool {
    std::env::var("VYP_NO_WHEEL_FILTER").is_ok_and(|v| v == "1")
}
...
pub fn filter_versions(
    ...
    let target_python = match env.python_full_version.parse::<VypVersion>() {
        Ok(v) => v,
        Err(_) => return,
    };
    versions.retain(|v| version_is_viable(wheel_info.get(v).map(|w| w.as_slice()), &tags, &target_python));
}
